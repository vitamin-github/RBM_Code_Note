...
def train_rbm():
    ...
    train_set=numpy.load('/home/min/bigdata/data/皮肤科数据集/data.npy')
   ...
   # print(rbm.W.get_value())
    ...
    for i in range(max_user_id):
        v0=numpy.copy(new_train_set[i,:]).reshape(1,210)
        m0=numpy.copy(new_train_mask[i,:]).reshape(1,210)
        v1=reconf(v0,m0)
        result=v1 if result is None else numpy.concatenate([result,v1],axis=0)
    print(result.shape)
    numpy.save('process_mat',result)
    numpy.save('costs',mean_costs)
